...
@{
    ...
    #  GENERATED ARTIFACTS
    #
    #  Files the install writes for the tools to read, because the tools cannot
    #  read what the repository keeps. This is machine output: regenerated by
    #  every apply, never edited by hand, never committed.
    #
    #  Kind names what the content is:
    ...
    #
    #  They deliberately land outside the linked configuration directory.
    #  Writing them inside would put generated output into the repository,
    #  which is the one thing a declared-state repository must never contain.
    # ------------------------------------------------------------------------
    GeneratedArtifacts = @(
        @{
    ...
    #  AGENT STATUS
    #
    #  Where the status line command writes what the agent knows about itself
    #  (model, context used, limits), one Lua file per project, and where the
    #  WezTerm status bar reads it from. Written while a workstation is open,
    #  never by an apply; removed by an uninstall. Never committed.
    # ------------------------------------------------------------------------
    AgentStatus = @{
        Name          = 'Agent status'
